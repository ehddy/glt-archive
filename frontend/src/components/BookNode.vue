<template>
  <div
    class="book-node"
    :class="[coverUrl ? 'book-node--cover' : `book-node--${variant}`]"
    :style="nodeStyle"
  >
    <div v-if="coverUrl" class="node-core node-core--cover">
      <img :src="coverUrl" :alt="`${title} 표지`" class="node-cover" />
      <span class="cover-spine-edge" aria-hidden="true" />
    </div>

    <div v-else class="node-core">
...
export default {
  ...
  computed: {
    ...
    nodeStyle() {
      return { '--hub-color': this.hubColor }
    },
  },
}
</script>
...
.book-node--cover:hover .node-core,
